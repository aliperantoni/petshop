...
const ListaCatAdmin = () => {
  ...
  const excluir = (CategoriaDel) => {
    api.delete(`categorias/${CategoriaDel.id}/`).then(() => {
      const listaCategorias = categorias.filter(
        (categoria) => categoria.id !== CategoriaDel.id
      );
        ...
        <tbody>
          {categorias.map((categoria) => (
            <tr key={categoria.id}>
              ...
              <td
                colSpan='2'
                className='tabela__coluna--m tabela__alinhamento--direita'
              >
